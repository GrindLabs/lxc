...
function install_vllm_openvino() {
  ...
  pct exec "${CTID}" -- bash -c ". /etc/os-release; \
    if [[ \"\${VERSION_CODENAME}\" == \"noble\" ]]; then \
      ...
      dpkg -i *.deb || apt-get install -y --fix-broken; \
      apt-get install -y intel-igc-core intel-igc-opencl intel-ocloc intel-opencl-icd libigdgmm12 libze-intel-gpu1; \
    else \
      echo \"Skipping Intel compute runtime .deb installs on \${VERSION_CODENAME}.\"; \
    fi"
...
EOF"
  ...
  pct exec "${CTID}" -- bash -c "git clone https://github.com/vllm-project/vllm.git /tmp/vllm && cd /tmp/vllm && \
    ...
    /opt/vllm-venv/bin/python benchmarks/benchmark_throughput.py --model meta-llama/Llama-2-7b-chat-hf --dataset benchmarks/ShareGPT_V3_unfiltered_cleaned_split.json"
